...
namespace sibr { 
	...
	public:
		...
		GaussianView(const sibr::BasicIBRScene::Ptr& ibrScene, uint render_w, uint render_h, const char* file, 
			bool* message_read, int sh_degree, const char* basis_file_path = nullptr, bool white_bg = false, bool useInterop = true, bool useBasis = false, int device = 0);
	...
	protected:
		...
		// basis model data
		CudaRasterizer::BasisModelInfo* basis_info = nullptr;
		CudaRasterizer::BasisModelInfo* basis_info_cuda = nullptr;
		uint8_t* basis_cuda = nullptr; // each basis value is a 8 bit uint
