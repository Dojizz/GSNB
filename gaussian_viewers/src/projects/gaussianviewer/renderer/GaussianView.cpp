...
// by default, T should be uint8_t
struct BasisValues
{
	uint8_t basis[16];
};
...
// load the basis pic from the path, pic name is basis.png
void loadBasis(const char* basis_path,
	std::vector<BasisValues>& basis, CudaRasterizer::BasisModelInfo* basis_info)
{
	// suppose we can read the image and convert them to vector
	basis_info->coff_num = 16;

	// load png to basis
	std::string pngname(basis_path);
	pngname += "basis.png";

	// png shape is (height * 16) * height
	cv::Mat image = cv::imread(pngname, cv::IMREAD_UNCHANGED);
	if (image.empty()) {
		std::cerr << "basis png not exists: " << pngname << std::endl;
		return;
	}
	basis_info->basis_image_height = image.rows;
	if (image.cols != basis_info->basis_image_height * basis_info->coff_num) {
		printf("basis png shape incorrect: (%d, %d)", image.cols, image.rows);
		return;
	}
	basis_info->basis_image_width = basis_info->basis_image_height; // by defualt, the image width = height
	basis_info->pixel_count = basis_info->basis_image_width * basis_info->basis_image_height;
	basis.resize(basis_info->pixel_count); // allocate space
	
	// read row by row
	for (int i = 0; i < basis_info->basis_image_height; i++) {
		for (int j = 0; j < basis_info->basis_image_width; j++) {
			// deal with 16 channels
			for (int k = 0; k < basis_info->coff_num; k++) {
				cv::Vec3b pixel = image.at<cv::Vec3b>(i, j + k * basis_info->basis_image_width);
				basis[i * basis_info->basis_image_width + j].basis[k] = pixel[0];
			}
		}
	}
	std::cout << "Basis model load complete.." << std::endl;
}
...
sibr::GaussianView::GaussianView(const sibr::BasicIBRScene::Ptr & ibrScene, uint render_w, uint render_h, const char* file,  
	bool* messageRead, int sh_degree, const char* basis_file_path, bool white_bg, bool useInterop, bool useBasis, int device) :
	_scene(ibrScene),
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	// Load the basis model data
	std::vector<BasisValues> basis;
	if (useBasis) {
		basis_info = new CudaRasterizer::BasisModelInfo();
		loadBasis(basis_file_path, basis, basis_info);
	}
	...
	if (useBasis) {
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&basis_cuda, sizeof(BasisValues) * basis_info->pixel_count));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(basis_cuda, basis.data(), sizeof(BasisValues) * basis_info->pixel_count, cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&basis_info_cuda, sizeof(CudaRasterizer::BasisModelInfo)));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(basis_info_cuda, basis_info, sizeof(CudaRasterizer::BasisModelInfo), cudaMemcpyHostToDevice));
	}
