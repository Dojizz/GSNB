...
int main(int ac, char** av) 
{
	...
	// get basis model path
	std::string basis_pic_path = "";
	if (myArgs.useBasisModel) {
		basis_pic_path = myArgs.modelPath.get();
		if (basis_pic_path.back() != '/')
			basis_pic_path += "/";
		basis_pic_path += "basis_model";
		if (!myArgs.iteration.isInit())
		{
			basis_pic_path += "/" + findLargestNumberedSubdirectory(basis_pic_path) + "/";
		}
		else
		{
			basis_pic_path += "/iteration_" + myArgs.iteration.get() + "/";
		}
	}
	...
	// Create the ULR view.
	GaussianView::Ptr	gaussianView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile.c_str(),  &messageRead, sh_degree, basis_pic_path.c_str(), white_background, !myArgs.noInterop, myArgs.useBasisModel, device));
